...
import { RatePlanQueryDto } from '../dto'
import { PaginationDto } from '../../../common/dto/pagination.dto'

@Injectable()
export class RatePlanRepository extends Repository<RatePlan> {
  constructor(
    @Inject(repositories.RATE_PLAN_REPOSITORY)
    private readonly _: Repository<RatePlan>,
  ) {
    super(_.target, _.manager, _.queryRunner)
  }

  async findByFiltersPaginated(payload: RatePlanQueryDto) {
    const { page, pageSize, search } = payload

    const pageNumber = page ?? 0
    const take = pageSize ?? 10
    const skip = Math.max(0, pageNumber) * take
    const query = this.createQueryBuilder('rate_plan')
