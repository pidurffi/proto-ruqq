...
import { QuotesQueryDto } from '../dto'
import { PaginationDto } from '../../../common/dto/pagination.dto'

@Injectable()
export class QuotesRepository extends Repository<Quotes> {
  constructor(
    @Inject(repositories.QUOTES_REPOSITORY)
    private readonly _: Repository<Quotes>,
  ) {
    super(_.target, _.manager, _.queryRunner)
  }

  async findByFiltersPaginated(payload: QuotesQueryDto) {
    const { page, pageSize, search } = payload

    const pageNumber = page ?? 0
    const take = pageSize ?? 10
    const skip = Math.max(0, pageNumber) * take
    const query = this.createQueryBuilder('quotes')
