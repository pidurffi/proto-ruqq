...
function createTenantAwareRepository(dataSource: DataSource, tenantService: TenantService) {
  return new Proxy(dataSource.getRepository(OccupancyRateModifiers), {
    get(target, prop, receiver) {
      ...
      if (typeof (target as any)[prop] === 'function' && 
        ...
        return function(...args: any[]) {
          ...
          console.log(`[OccupancyRateModifiersTenantProvider] Executing ${String(prop)} on schema: ${tenantContext.schema}`)
          ...
          if (prop === 'createQueryBuilder') {
            ...
            queryBuilder.from(`${tenantContext.schema}.occupancy_rate_modifiers`, args[0] || 'occupancyRateModifiers')
            return queryBuilder
          }
          ...
          return dataSource.transaction(async manager => {
            ...
            const repoWithSchema = manager.getRepository(OccupancyRateModifiers)
...
/**
 * Provider tenant-aware para OccupancyRateModifiers
 */
export const OccupancyRateModifiersTenantProviders = [
  {
    provide: repositories.OCCUPANCY_RATE_MODIFIERS_REPOSITORY,
    useFactory: (tenantService: TenantService, dataSource: DataSource) => {
      console.log(`[OccupancyRateModifiersTenantProvider] Creating repository for schema: ${tenantService.getActiveTenant().schema}`)
