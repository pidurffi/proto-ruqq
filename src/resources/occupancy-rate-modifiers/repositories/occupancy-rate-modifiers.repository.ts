...
import { OccupancyRateModifiersQueryDto } from '../dto'
import { PaginationDto } from '../../../common/dto/pagination.dto'

@Injectable()
export class OccupancyRateModifiersRepository extends Repository<OccupancyRateModifiers> {
  constructor(
    @Inject(repositories.OCCUPANCY_RATE_MODIFIERS_REPOSITORY)
    private readonly _: Repository<OccupancyRateModifiers>,
  ) {
    super(_.target, _.manager, _.queryRunner)
  }

  async findByFiltersPaginated(payload: OccupancyRateModifiersQueryDto) {
    const { page, pageSize, search } = payload

    const pageNumber = page ?? 0
    const take = pageSize ?? 10
    const skip = Math.max(0, pageNumber) * take
    const query = this.createQueryBuilder('occupancy_rate_modifiers')
