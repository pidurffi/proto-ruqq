...
import { SectionImgDto, SectionImgQueryDto } from '../dto/section-img.dto'
...
@Injectable()
export class SectionImgService extends BaseEntityService<SectionImg> {
  constructor(
    @Inject(SectionImgRepository)
    private readonly repository: SectionImgRepository,
  ...
  protected getRepository(): SectionImgRepository {
    return this.repository
  }

  async createImgsRoom(sectionImgDto: SectionImgDto, uid: string, files: Express.Multer.File[]) {
    // Verifica que exista la habitación
    const room = await this.sectionService.findByIdOrFail(sectionImgDto.sectionId)
    if (!room) {
  ...
  async findAllWithFilterPaginated(payload: SectionImgQueryDto) {
